from django.db import models


class CoilDimensions(models.Model):
    coreDia = models.IntegerField(default=0)
    coreGap = models.FloatField(null=True, blank=True)
    lVID = models.IntegerField(default=0)
    lVRadial = models.IntegerField(default=0)
    lVOD = models.IntegerField(default=0)
    lVHVGap = models.FloatField(null=True, blank=True)
    hVID = models.IntegerField(default=0)
    hVRadial = models.IntegerField(default=0)
    hVOD = models.IntegerField(default=0)
    hVHVGap = models.FloatField(null=True, blank=True)
    coilCoilGap = models.FloatField(null=True, blank=True)
    corseID = models.IntegerField(null=True, blank=True)
    corseRadial = models.IntegerField(null=True, blank=True)
    corseOD = models.IntegerField(null=True, blank=True)
    corseGap = models.FloatField(null=True, blank=True)
    fineID = models.IntegerField(null=True, blank=True)
    fineRadial = models.IntegerField(null=True, blank=True)
    fineOD = models.IntegerField(null=True, blank=True)
    fineGap = models.FloatField(null=True, blank=True)
    outerID = models.IntegerField(null=True, blank=True)
    outerRadial = models.IntegerField(null=True, blank=True)
    outerOD = models.IntegerField(null=True, blank=True)
    outerGap = models.FloatField(null=True, blank=True)
    centerDistance = models.IntegerField(null=True, blank=True)
    outermostOD = models.IntegerField(null=True, blank=True)
    outermostWinding = models.CharField(max_length=32, blank=True, default="")
    activePartSize = models.CharField(max_length=255, blank=True, default="")

    def __str__(self):
        return f"CoilDimensions {self.pk or 'unsaved'}"
